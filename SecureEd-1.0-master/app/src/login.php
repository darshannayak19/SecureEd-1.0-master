...
try {
    ...
    function email_validation($myusername) {
        return (!preg_match(
    "^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$^", $myusername))
            ? FALSE : TRUE;
    }

    function isValidPassword($mypassword) {
       if (!preg_match('/(?=.*[0-9])(?=.*[A-Z]).{8,20}$/', $mypassword)){
         return FALSE;
       }else{
        return TRUE;
       }
    ...
    // Function call
    if(!email_validation($myusername) || !isValidPassword($mypassword)) {
